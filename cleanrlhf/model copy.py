...
class CausalSelfAttention(nn.Module):
    ...
    def __call__(self, x: jnp.array, attn_pdrop_key: jax.random.KeyArray, resid_pdrop_key: jax.random.KeyArray):
        ...
        c_attn = nn.Dense(3 * C, use_bias=self.bias)(x)  # (B, T, 3 * C), `c_attn` means `concatenated attention`
        ...
        c_proj = nn.Dense(C, use_bias=self.bias)(y) # (B, T, C)
        y = dropout(c_proj, rate=self.resid_pdrop, key=resid_pdrop_key)  # (B, T, C)
        return y
...
class Block(nn.Module):
    ...
    def __call__(self, x: jnp.array, key: jax.random.KeyArray):
        ...
        oldx = x
        x = nn.LayerNorm(self.embd_dim, use_bias=self.bias)(x)
        x = CausalSelfAttention(self.embd_dim, self.n_head, self.attn_pdrop, self.resid_pdrop, self.block_size, self.bias)(
        ...
        oldx = x
        x = nn.LayerNorm(self.embd_dim, use_bias=self.bias)(x)
        x = nn.Dense(4 * self.embd_dim, use_bias=self.bias)(x)
        x = NewGELU()(x)
        x = nn.Dense(self.embd_dim, use_bias=self.bias)(x)
...
class GPT(nn.Module):
    ...
    def __call__(self, idx: jnp.array, targets: jnp.array, key: jax.random.KeyArray):
        ...
        x = nn.LayerNorm(self.c.embd_dim, use_bias=self.c.bias)(x)
...
if __name__ == "__main__":
    ...
    block_y, key = block.apply(block_params, x, key)
